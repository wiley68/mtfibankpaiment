            ...
            if (this.readyState == 4 && this.status == 200) {
                try {
                    ...
                    if (fibank_is_visible && options) {
                        ...
                        const mtfibank_vnoska_second_txt =
                        ...
                        if (mtfibank_vnoska_txt) {
                            mtfibank_vnoska_txt.textContent =
...
function fibank_pogasitelni_vnoski_input_change() {
    ...
    xmlhttpro.onreadystatechange = function () {
        if (this.readyState == 4) {
            ...
            var fibank_vnoska = parseFloat(
                JSON.parse(this.response).fibank_vnoska
            );
            var fibank_gpr_res = parseFloat(
                JSON.parse(this.response).fibank_gpr
            );
            var fibank_glp_res = parseFloat(
                JSON.parse(this.response).fibank_glp
            );
            ...
            if (fibank_is_visible) {
                if (options) {
                    ...
                    const fibank_vnoska_input =
                        document.getElementById("fibank_vnoska");
                    const fibank_gpr = document.getElementById("fibank_gpr");
                    const fibank_glp = document.getElementById("fibank_glp");
                    const fibank_obshtozaplashtane_input =
                        document.getElementById("fibank_obshtozaplashtane");
                    const mtfibank_vnoska_txt = document.getElementById(
                        "mtfibank_vnoska_txt"
                    );
                    if (mtfibank_vnoska_txt) {
                    ...
                    if (fibank_eur == 1) {
                        ...
                        fibank_obshtozaplashtane_input.value =
                            ...
                            "/" +
                            ((fibank_vnoska * fibank_vnoski) / 1.95583).toFixed(
                                2
                            );
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (btn_fibank !== null) {
        ...
        const fibank_back_credit =
            document.getElementById("fibank_back_credit");
        ...
        const fibank_maxstojnost =
            document.getElementById("fibank_maxstojnost");
        ...
        btn_fibank.addEventListener("click", (event) => {
            const fibank_eur = parseInt(
                document.getElementById("fibank_eur").value
            );
            const fibank_currency_code = document.getElementById(
                "fibank_currency_code"
            ).value;
        ...
        const fibankSpanOffers = document.querySelectorAll(
            '[itemprop="offers"]'
        );
